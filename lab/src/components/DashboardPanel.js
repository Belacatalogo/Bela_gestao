  ...
  return `
    <section class="panel-card dashboard-panel">
      <div class="panel-title-row">
        <div><h2>Dashboard LAB</h2><p>Resumo rápido de produtos, vendas, pagamentos e alertas do catálogo fictício.</p></div>
...
function renderCatalogUploadMapPanel(report) {
  return `
    <section class="panel-card catalog-upload-map-panel">
      ...
      <div class="mini-grid four-stats">
        <div class="mini-stat"><strong>${report.realCatalogConnected ? 'sim' : 'não'}</strong><span>catálogo real</span></div>
        <div class="mini-stat"><strong>${report.realUploadConnected ? 'sim' : 'não'}</strong><span>upload real</span></div>
        <div class="mini-stat"><strong>${report.fields.length}</strong><span>campos</span></div>
        <div class="mini-stat"><strong>${report.realWritesBlocked ? 'sim' : 'não'}</strong><span>escrita bloqueada</span></div>
      </div>
